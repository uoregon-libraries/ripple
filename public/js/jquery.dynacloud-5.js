...
*/
/*
  Modified by William Myers
  - adjust word length to be 3 letters or longer
  - added maxFontSize as a property
   * Default: 5
   * This will limit the word size to 5em
 */
...
jQuery.fn.dynaCloud = function(outElement) {
 ...
 return this.each(function() {
  ...
  if (jQuery.wordStats && jQuery.dynaCloud.wordStats) {
   ...
   // Modified to allow lowercase words to be evaluated - William Myers
   //var word = /^[a-z\xE4\xF6\xFC]*[A-Z\xC4\xD6\xDC]([A-Z\xC4\xD6\xDC\xDF]+|[a-z\xE4\xF6\xFC\xDF]{2,})/;
   var word = /^[a-z\xE4\xF6\xFC]{2,}/i;
   console.log("elems", elems);
   console.log("stop Regex", jQuery.dynaCloud.stopwords);
  ...
  console.log("Cloud List", cl);

  for (var i = 0; i < l; ++i) {
    var fontSize = Math.ceil( (cl[i].count / max) * jQuery.dynaCloud.scale );
    var adjFontSize = (fontSize > 5) ? 5 : fontSize;
    console.log(adjFontSize, " - ", fontSize);
    out.append('<a href="#' + cl[i].el + '" style="font-size: ' + adjFontSize + 'em"><span>' + cl[i].el + '</span></a> &nbsp; ');
  }
