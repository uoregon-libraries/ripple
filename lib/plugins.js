...
plugin.validHookNames = [
  ...
  /**
   * Hooks associated with plugin load, save, and display
   * 
  ...
  /**
   * Hooks associated with account authenication
   * 
...
/**
 * Logger API Module handles output to console and logs. It is a direct map of <a href="https://github.com/bevry/caterpillar#usage">Catipillar npm module</a>.
 * 
 * @class plugin-server.logger
 * @title logger
 */
...
DB.init.collection('plugin_config').find({status:"1"}).toArray(function(err, documents){
  for(item in documents){
    ...
    /**
     * Hook to load plugin config.
     *
     * @event loadConfig
     * @for plugin-server.plugin
     * @param {Object} config An object containing the current configuration of the plugin
