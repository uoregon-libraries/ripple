...
AM.update = function(o, newData, callback) {   
  ...
  log("Update Obj", util.inspect(o) );
  log("Posted Data", util.inspect(newData) );
  if (newData.pass == ''){
...
AM.passwordRecovery = function(req, callback){
  ...
  log("Password Recovery for",emailAddress);
  // Query for email
  AM.accounts.findOne({email:emailAddress}, function(err, doc){
    if(err) {
      ...
      log('Found Email sending password link for ', emailAddress);
      ...
      generatePasswordResetDB(doc, function(err, resetObj){
        if( !err ) {
          ...
          log('Password Email sent to ' + emailAddress, link);
      ...
      log('[AM.passwordRecovery] User not found', emailAddress);
...
var generatePasswordResetDB = function(userObj, callback){
  ...
  log(fnName + " User ID", userObj._id );
  // Check for userObj
  if( !userObj ) {
  ...
  log("[AM>generatePasswordResetDB] Reset Doc", util.inspect(resetDoc));
  // Save Record in DB
...
AM.setPassword = function(guid, newpass, callback) {
  ...
  log("[AM.setPassword] GUID - " + guid, newpass);
  // Verify GUID
  if(!guid){
...
AM.routeAuthUser = function(o, req, res, callback){
  if (o != null){
    ...
    log("[AM.routeAuthUser] " + o.user, req.url);
    // AM.renderAuthRoute(route, pageData, res);
    callback();
  } else{
    log("AM.routeAuthUser", "Show Login");
    res.redirect('/');
  } 
}
...
authCheckSession = function(authObj, userObj, req, res, callback){
  ...
  DB.init.collection('sessions').findOne({'_id':elemID}, function(err, record){
    if( !err ) {
      ...
      log('Records', util.inspect(record));
      log('User', userObj._id );
      log('Author', record.author );
...
authCheckPermissions = function(authObj, userObj, req, res, callback){
  ...
  AM.permissions.findOne({name:authObj.component}, function(err, record){
    ...
    roles.forEach(function(item, index){ 
      console.log("roles.forEach item :: ",item);
      console.log("roles.forEach index :: ", index)
      // Exit loop if found role match
      console.log(permission);
      console.log(userObj);
      console.log(typeof userObj.roles === 'undefined')
      if( permission ) return;
        ...
        userObj.roles.forEach(function(authRole){
          console.log("userObj.roles.forEach item :: ",item);
