...
GLOBALS.helperFn.setSessionExpire = function(session){
  // Check for data
  log("session",util.inspect(session) );
  if(!session) return;
...
GLOBALS.helperFn.randomAlphaNum = function(length) {
  ...
  log("[GLOBALS.helperFn.randomAlphaNum] UUID Generated", returnStr)
  return returnStr;
};

GLOBALS.helperFn.stripTags = function(source) {
  //log("[GLOBALS.helperFn.stripTags] args", util.inspect(arguments) );
  var tags = /<(?:.|\n)*?>/gm
    , altered;
...
GLOBALS.error.callbackAndLog = function(errMsg, errTitle, fnName, callback){
  //log("[GLOBALS.error.callbackAndLog] Params", util.inspect(arguments) );
